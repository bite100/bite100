...
- 环境变量：`VITE_NETWORK`（sepolia/mainnet/polygon）、`VITE_NODE_API_URL`（单或多节点）。  
- 钱包：依赖设备浏览器内的钱包扩展（如 MetaMask 移动版、内嵌浏览器）；无扩展时需用 WalletConnect 等（当前前端为 EIP-1193，Phase 4 可扩展 WalletConnect）。
