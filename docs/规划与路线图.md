...
| **Phase 4** | 手机端、跨链、治理代币/DAO、第三方接入 | 🔶 部分：Web 已移动端适配 + PWA；Electron 桌面版；主网/多链配置；原生 App、跨链、DAO 代币未做 |
...
| ~~贡献看板~~ | ✅ 已做：前端贡献看板，按地址查询最近两周未领取/已领取，支持领取；链上领取截止 14 天 |
...
| 治理代币 | 若采用原生治理代币，设计发行、质押、与贡献/治理的绑定 |
...
- 跨链：多链部署 + 前端链切换 + 文档
- DAO：治理代币、提案类型扩展、执行延迟（Timelock）等
