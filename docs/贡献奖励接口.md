...
## 3.3 奖励池与兑换规则（稳定币计价、价值按周期锁定）

- **贡献周期与结算**（与 [Phase3 §5.2](./Phase3-设计文档.md#52-结算周期) 一致）：**贡献周期 7 天**（UTC 自然周）；**结算滞后一周**——当周产生的贡献对应**下一周**的奖励池（当周手续费收入作为下周可分配池）。节点在**周期结束后**提交证明，合约汇总该周期 totalScore 与各地址 contributionScore；owner 注入该周期奖励池并**确定该周期的「每贡献分价值」**后，节点可 **claim**。**上线奖励**：开发者上线奖励（如周期 "launch"）同样适用「按发放时锁定的价值」领取。
...
- **每个周期一个贡献分价值**：在**该周期结算/发放贡献分时**确定并锁定：
  - **可分配池价值（稳定币）** = 该周期注入的可分配奖励池价值（扣减储备后）；
  - **该周期总贡献分** = 该周期所有有效证明汇总后的 totalScore；
  - **该周期「每贡献分价值」** = 可分配池价值（稳定币）÷ 该周期 totalScore。
- **以后任何时候领取**：均按**发放贡献分时（该周期）锁定的价值**计算应得额度，不按领取时价格重算：
  - 某地址在该周期**应得稳定币价值（锁定）** = 该地址在该周期的 contributionScore × 该周期「每贡献分价值」；
  - 若用代币 A 领取：**领取数量** = 应得稳定币价值（锁定）÷ 代币 A 在**该周期结算时**的稳定币单价（即发放贡献分时采用的价格），**不是**领取时的市价。
- **效果**：同一周期内「1 贡献分值多少」在该周期确定后不变；晚领、早领拿到的**代币数量**都按周期价格算，保证领取的是**发放时的那份价值**。

### 领取限制与截止（重新规定）

- **领取限制**：可按应得额度**一次或分次**领取，不再设「单次不超过池子 10%」的上限；具体是否分次由合约实现与 gas 考虑决定。
- **领取截止**：**不设「周期结束 14 天后作废」**。任何时候领取，应得额度均按该周期锁定的「每贡献分价值」与周期结算时代币价格计算；未领部分一直有效，直至领完（合约需保证该周期池子或预留足够代币/资金用于兑付）。
- **未设置结束时间的周期**（如 "launch"）：同样适用「按发放时锁定的价值」；可随时领取。

### 结算价格与自由流动

- **结算时价格的计算（避免周末单点波动）**：奖励池及各币种在周末结算时若只取某一时刻价格，可能因波动导致价值相差巨大。采用**按日去极值再周平均**：对每个自然日，先去掉该日采样价中**最高的 5%** 与**最低的 5%**，对剩余价格取算术平均得到该日「日均价」；再对当周各日的日均价取**算术平均**，作为当周「平均价格」用于**该周期价值锁定**与领取时的代币数量计算。
- **自由流动额度**（可选）：若保留「自由流动」设计，允许当周奖励的一定比例（如 10%）按**领取时**市价兑换，其余仍按周期锁定价值；该比例可通过治理调整。当前合约实现中 `freeFlowBps` 已预留；若不做自由流动，则全部按周期锁定价值领取。
- **合约实现**：当前 ContributorReward 为「按周期、按代币设池」的简化版（`setPeriodReward(period, token, amount)` 每币种单独注入）。**新规则下**：周期结算时需记录该周期「可分配池价值（稳定币）」及该周期各币种对稳定币的**结算价格**（或仅记录每贡献分可领各代币数量），领取时按**周期价格**算出代币数量并扣减池余额；不按领取时预言机价格重算。
- **自由流动比例治理**：`freeFlowBps` 可通过**治理投票**调整；投票通过后由合约执行 `setFreeFlowBps(uint16)`，上限可由合约或治理约定（如 5%～30%）。
...
**贡献分与应得奖励均保存在链上**（ContributorReward 合约），与前端或节点 UI 无关：

- **领取入口不唯一**：用户（例如中继节点挂机获得的贡献分）可在**本 DEX 前端**领取，也可在**任意其他**连接同一链、同一 ContributorReward 合约的 dApp、钱包或脚本中领取。
- **方式**：用**同一领奖钱包**连接该链，调用合约的 `claimReward(period, token)` 并签名即可；奖励会转入该钱包。
- **第三方接入**：只需合约地址、链 RPC、以及 ABI 中的 `claimReward`、`claimable(period, token, account)`、`getContributionScore(period, account)` 等。合约地址与接口见 [API-接口说明 §2.6](./API-接口说明.md#26-contributorreward贡献奖励phase-2-m4)。
- **效果**：挂机用户无需一定打开本前端，可在自己常用的钱包、聚合 dApp 或自建小工具中兑换奖励。
