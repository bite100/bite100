...
**规则**：
- **仅开发者积分**按 **1 积分 = 1 USDT**（或等值稳定币）兑换发放
- 上线前累计，主网上线时发放
- 最小贡献：1 PR 或 100 行代码
...
**来源**：
- 运行时长（每天在线计 1 节点积分）
- 撮合订单量（每 100 笔订单计 1 节点积分）
- 网络贡献（中继、存储等）

**规则**：
- 节点积分**不**按 1 积分 = 1 USDT；按总池中节点占比（如 30%）与积分权重分配
- 最小运行时长：7 天
- 防刷：需心跳验证、最小撮合量
...
**目标**：交易所记录钱包地址，开发者积分绑定到钱包，钱包地址关联的节点也可以领取。同一个钱包既能领开发者积分，也能领节点运行奖励，一键领取所有奖励。
...
创建 `frontend/src/components/RewardBinding.tsx`：
...
export function RewardBinding() {
  ...
  return (
    <div className="card">
      <h3 className="text-lg font-bold mb-3">奖励钱包绑定</h3>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            value={nodeId}
            onChange={(e) => setNodeId(e.target.value)}
            placeholder="12D3KooW...（可选）"
            className="input w-full"
          />
          <p className="text-xs text-gray-500 mt-1">
            如果运行节点，输入节点 ID 可同时领取节点奖励
          </p>
        </div>
...
节点启动时自动调用合约绑定（见前面的 NodeRegistry 实现）。
...
### 7.3 合约扩展（ContributorReward）

扩展 `contracts/src/ContributorReward.sol`：
...
/**
 * @title ContributorReward (扩展版)
 * @notice 统一奖励领取：开发者积分 + 节点奖励
 */
contract ContributorReward is Ownable {
    IERC20 public immutable rewardToken;  // USDT 或项目代币
    Governance public governance;
    
    // 积分记录
    mapping(address => uint256) public devPoints;          // 开发者积分
    mapping(address => uint256) public nodePoints;         // 节点积分
    mapping(address => uint256) public feeRewards;         // 手续费分成奖励
    
    // 节点绑定
    mapping(bytes32 => address) public nodeIdToWallet;    // libp2p nodeID hash => 钱包
    mapping(address => bytes32) public walletToNodeId;    // 钱包 => nodeID hash
    
    // 领取记录
    mapping(address => bool) public hasClaimedLaunchReward; // 是否已领取上线奖励
    ...
    /**
     * @notice 绑定钱包（开发者/节点运营者）
     * @param nodeId 节点 ID hash（libp2p peerID hash），非节点用户传 bytes32(0)
     * @param wallet 奖励接收钱包地址
     */
    ...
    /**
     * @notice 批量分配积分（仅 Governance）
     * @param wallets 钱包地址列表
     * @param devAmounts 开发者积分列表
     * @param nodeAmounts 节点积分列表
     */
    ...
    /**
     * @notice 一键领取所有奖励（开发者积分 + 节点积分 + 手续费分成）
     */
    function claimRewards() external {
        ...
        // 转账 USDT（仅开发者积分 1 积分=1 USDT；节点/手续费按池分配，此处为示意）
    ...
    /**
     * @notice 从 FeeDistributor 分配手续费分成（仅 Governance）
     */
...
创建 `frontend/src/components/UnifiedRewardClaim.tsx`：
  ...
  return (
    <div className="card">
      ...
      <div className="space-y-3">
        <div className="flex justify-between">
          <span className="text-gray-600">开发者积分：</span>
        ...
        <div className="flex justify-between">
          <span className="text-gray-600">节点奖励：</span>
          <span className="font-medium">
        ...
        <div className="flex justify-between">
          <span className="text-gray-600">手续费分成：</span>
          <span className="font-medium">
        ...
        {isSuccess && (
          <div className="bg-green-50 border border-green-200 rounded p-3">
            <p className="text-green-800 text-sm">
              ✅ 奖励已领取！已到账 {(Number(rewards.total) / 1e6).toFixed(2)} USDT
            </p>
          </div>
        )}
      </div>
...
2. **自动分配手续费分成**：
   - Governance 定期调用 `distributeFeeRewards`
   - 按节点贡献比例分配

### 7.6 防作弊机制

1. **绑定验证**：
   - 需钱包签名（`msg.sender == wallet`）
   - EIP-712 签名验证（可选）
...
*本文档提供上线奖励分发机制的完整实现方案，包括统一奖励领取机制，从节点绑定到批量发放，适合项目上线时的激励分发。*
