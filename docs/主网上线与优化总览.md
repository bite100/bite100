...
| 项目 | 路径 | 说明 |
|------|------|------|
| 生产 Docker | `docker-compose.prod.yml`、根目录 `.env.example` | `docker compose -f docker-compose.prod.yml --env-file .env up -d` |
| NodeRewards | `contracts/src/NodeRewards.sol` | 上线激励绑定、allocatePoints；部署见 [上线奖励分发机制实现指南](./上线奖励分发机制实现指南.md) |
| Snapshot | `scripts/snapshot.py` | 生成 snapshot.json 供 Governance allocatePoints |
| TVL | [TVL追踪建议](./TVL追踪建议.md)、`scripts/tvl_example.py` | DefiLlama/Dune/自建脚本 |
