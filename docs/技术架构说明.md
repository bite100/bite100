...
> 版本：v0.2  
> 更新日期：2025-02-07  
> 关联文档：[概念设计文档](./概念设计文档.md)、[API 接口说明](./API-接口说明.md)、[Phase2 设计文档](./Phase2-设计文档.md)
...
### 2.2 智能合约模块（Phase 1 已实现）

```
contracts/
├── Vault.sol              # 资产托管保险库
├── Settlement.sol         # 链下撮合交易结算
├── FeeDistributor.sol     # 手续费分配：开发者 1% 自动到账，其余 claim
├── AMMPool.sol            # AMM 恒定乘积交易池
├── interfaces/IERC20.sol
└── mock/MockERC20.sol     # 测试代币
```

**未来（Phase 3+）**：OrderBook、ContributorReward、TokenPair 等。

### 2.3 合约职责

| 合约 | 职责 | 核心方法 |
|------|------|---------|
| **Vault** | 托管用户资产，划转结算 | `deposit`, `withdraw`, `balanceOf` |
| **Settlement** | 链下撮合成交后的资产划转 | `settleTrade` |
| **FeeDistributor** | 接收手续费：1% 自动转开发者，其余按比例 claim | `receiveFee`, `setDeveloperAddress`, `claim`, `claimable` |
| **AMMPool** | AMM Swap、添加流动性 | `swap`, `addLiquidity`, `getAmountOut`, `reserve0`/`reserve1` |

**接口与地址详见** [API 接口说明](./API-接口说明.md)。

### 2.4 结算流程

**Phase 1（AMM 直连）**：
```
1. 用户 approve → AMMPool（或 Vault）
2. Vault：deposit / withdraw 托管资产
3. AMM：swap / addLiquidity 直接与池子交互，手续费自动转入 FeeDistributor
4. 贡献者 claim 手续费份额
```

**Phase 3（链下订单簿）**：
```
1. 用户 deposit 资产至 Vault
2. 链下撮合产生成交信息
3. 提交成交证明至 Settlement 合约
4. Settlement 调用 Vault.transferWithinVault 划转
5. 手续费转入 FeeDistributor
```
...
| 数据类型 | 保留期限 | 清理策略 |
|---------|---------|---------|
| 订单簿快照 | 滚动窗口 | 每 N 小时持久化，统一两周 |
| 历史成交 | 统一两周 | 按 `timestamp` 过滤，超期删除 |
| 订单变更日志 | 统一两周 | 按 `createdAt` 过滤，超期删除 |

**实现要点**：
- 存储节点定时任务：每日扫描，删除超期记录（默认两周）
...
**Phase 1（当前）**：
```
1. 连接钱包（MetaMask 等 EIP-1193 提供方）
2. 切换到 Sepolia 测试网
3. 前端直连链上合约：Vault 存提、AMMPool Swap / 添加流动性
4. 交易由钱包签名，链上直接执行
```
...
```python
# 伪代码：存储节点清理超期数据（默认两周）
RETENTION_MONTHS = 6  # 电脑端 6，手机端 1
...
def sync_trades(since, until):
    max_age = now() - timedelta(days=RETENTION_MONTHS * 30)
    if since < max_age:
...
| [API 接口说明](./API-接口说明.md) | 合约 ABI、链配置、前端集成、Phase 2 同步协议 |
| [Phase2 设计文档](./Phase2-设计文档.md) | 节点软件、数据同步、贡献度量与里程碑 |

---

*本文档以当时修改为准，随 Phase 推进随时更新设计文档。*
