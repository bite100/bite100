...
本文档说明如何将 P2P 交易所部署到 **Ethereum 主网**（chainId 1）。主网使用真实 ETH 和代币，请务必在测试网充分验证后再部署。**若需更低 gas 成本，可部署到 Polygon 主网**，见 [Polygon 部署指南](./Polygon部署指南.md)。
...
在 `.env` 中配置 `ETHERSCAN_API_KEY` 后，部署时添加 `--verify`：
