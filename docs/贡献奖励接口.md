...
| `nodeType` | `uint8` | 0=relay, 1=storage（或扩展） |
| `signature` | `bytes` | ECDSA 签名：对 `keccak256(abi.encodePacked(period, uptime, storageUsedGB, storageTotalGB, bytesRelayed, nodeType))` 的签名，且 `ecrecover(...) == msg.sender` |
...
- **按节点类型**：  
  - relay：仅 uptime + bytesRelayed（storage 项为 0）。  
  - storage：uptime + storage 项（bytesRelayed 为 0）。  
  - 若未来支持「混合节点」，可同时计三项。
...
- **提案通过规则**：**最近活跃人员**（过去 4 周内有 `submitProof`，不统计投票）中，**同意超过 50%** 即通过。细则见 [概念设计文档 §4.4](./概念设计文档.md#44-治理提案规则)。
...
- **已实现**：  
  1. **合约**：`contracts/src/ContributorReward.sol`，含 `submitProof`（ECDSA 校验与贡献分计算）、`setPeriodReward`、`claimReward` / `claimable`、`getContributionScore` / `getPeriodTotalScore`。部署脚本 `runContributorReward()`。  
  2. **节点端**：`node/internal/reward` 包（构建 digest、ECDSA 签名、编码 calldata）；`node/cmd/submitproof` 命令行：读证明 JSON，用 EVM 私钥签名并发送交易。用法：`go run ./cmd/submitproof -proof <path> -contract <addr> -rpc <url> -key <hex>` 或环境变量 `REWARD_ETH_PRIVATE_KEY`。  
  3. 部署与测试网验证后，在 [Phase2-设计文档](./Phase2-设计文档.md) 与 [API-接口说明](./API-接口说明.md) 中链到本接口并勾选 M4 对应项。
