...
| **数据同步与存储** | 节点间订单簿/成交数据同步协议，存储节点持久化与保留期清理（默认 6 个月） |
...
- 约束：存储节点仅返回保留期内的数据（默认 6 个月）；`since < now - retention` 的请求视为 `since = now - retention`。
...
- **保留期**：电脑端节点存 6 个月、手机端节点存 1 个月（本节点配置 `storage.retention_months`），超期删除。
- **清理**：各存储节点定时任务（如每日）按本节点保留月数删除超期成交与订单簿快照。
- **调取**：手机端需超过 1 个月数据时，向电脑端节点（保留 6 个月）发起 SyncTrades 拉取。
...
- [x] 保留期清理：定时清理任务（默认 6 个月），同步接口只返回保留期内数据。
...
- [ ] 定义 ContributorReward 或扩展 FeeDistributor 的接口（提交证明、查询积分/奖励）；**接口草稿见 [贡献奖励接口.md](./贡献奖励接口.md)**。
- [ ] （可选）实现并部署合约，节点端提交证明的调用或脚本。
- [ ] 节点部署文档：安装、配置、Bootstrap 列表、防火墙建议（初稿见 [节点部署.md](./节点部署.md)）。
