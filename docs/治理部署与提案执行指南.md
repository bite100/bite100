...
- 部署 Governance 并绑定 Settlement、AMMPool、ContributorReward、FeeDistributor
- 部署 TokenRegistry、ChainConfig 并设置 `GOVERNANCE_ADDRESS`

**FeeDistributor 治理**：绑定后，可通过 Governance 提案执行 `FeeDistributor.setRecipients()` 或 `setDeveloperAddress()` 修改手续费分成比例与对象（见清单 5.1）。
...
**一键执行（推荐）**：在项目根目录下执行脚本（默认 Sepolia、proposalId=0）：
...
**Sepolia 全流程速查**：[治理闭环-Sepolia操作清单](./治理闭环-Sepolia操作清单.md)。
