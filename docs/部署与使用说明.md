...
**生产部署（Docker Compose 生产版）**：根目录提供 `docker-compose.prod.yml` 与 `.env.example`。用法：`cp .env.example .env` 并编辑 `NETWORK`、`REWARD_WALLET`、`BOOTSTRAP_NODES` 等；然后执行 `docker compose -f docker-compose.prod.yml --env-file .env up -d`。**领奖地址必填**：可通过 `.env` 的 `REWARD_WALLET`、配置文件或启动参数 `-reward-wallet` 设置；未设置则节点拒绝启动。若需自动提交证明领奖，再配置 `REWARD_ETH_PRIVATE_KEY` 并用定时任务跑 submitproof。**Docker 无人值守自动更新**：使用 [scripts/docker/README.md](../scripts/docker/README.md) 中的脚本 + cron（Linux）或计划任务（Windows），可定时拉代码、重新构建并重启容器，无需人工干预。可选 `--profile watchtower` 启用 Watchtower 自动更新镜像。详见 [主网上线与优化总览](./主网上线与优化总览.md)。
