...
param(
    ...
    [string]$RpcUrl = $env:BASE_RPC_URL,
    [string]$Token0Address = $env:BASE_TOKEN0_ADDRESS,
    [string]$Token1Address = $env:BASE_TOKEN1_ADDRESS
)
...
if (-not $RpcUrl) {
    Write-Host "错误: 请设置 BASE_RPC_URL 环境变量（如 https://mainnet.base.org）" -ForegroundColor Red
    exit 1
}

if (-not $Token0Address -or -not $Token1Address) {
    Write-Host "错误: 请设置 BASE_TOKEN0_ADDRESS 和 BASE_TOKEN1_ADDRESS 环境变量" -ForegroundColor Red
    exit 1
}

Write-Host "=== 部署到 Base 主网 ===" -ForegroundColor Cyan
...
$scriptPath = "script/DeployBase.s.sol"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "`n✅ Base 部署成功!" -ForegroundColor Green
